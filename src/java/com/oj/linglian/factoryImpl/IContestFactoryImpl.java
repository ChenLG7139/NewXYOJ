...
public class IContestFactoryImpl implements IServletFactory {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response,
        ...
        if (tMap != null) {
            ...
            if ("-99".equals(user.getLvl())) {
                Rank r = new Rank();
                r.setContestId(c.getContestId());
                List<Rank> list = DBUtil.getObjectLimit("rank", "score", "desc", r, 0, 10000);
            ...
            } else if (StringUtil.isSmallLong(c.getEndTime(), String.valueOf(new Date().getTime()))) {
                request.getSession().setAttribute("info", "竞赛已经结束");
                ServletUtil.forward(request, response, servlet, "get_from");
            } else if (StringUtil.isBigLong(c.getStartTime(), String.valueOf(new Date().getTime()))) {
                request.getSession().setAttribute("info", "竞赛还没有开始");
                ServletUtil.forward(request, response, servlet, "get_from");
            } else {
                Rank r = new Rank();
                r.setContestId(c.getContestId());
